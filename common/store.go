...
type EventStore interface {
	...
	//SaveEventNotifyByTx(txHash Uint256, notify *event.ExecuteNotify) error
	//Save transaction hashes which have event notify gen
	SaveEventNotifyByBlock(height uint32, txHashs []Uint256)
	//GetEventNotifyByTx return event notify by transaction hash
	//GetEventNotifyByTx(txHash Uint256) (*event.ExecuteNotify, error)
