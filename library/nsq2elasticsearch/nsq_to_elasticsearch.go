...
var (
	...
	channel     = flag.String("channel", "nsq2elasticsearch", "nsq channel")
...
// nsq2elasticsearch --topic logging --channel logging --lookupd-http-address 127.0.0.1:4161 --elasticsearch-addrs http://127.0.0.1:9200
func main() {
	...
	if *showVersion {
		fmt.Printf("nsq2elasticsearch v%s\n", Binary)
		return
	}
	...
	cfg.UserAgent = fmt.Sprintf("nsq2elasticsearch/%s go-nsq/%s", Binary, nsq.VERSION)
