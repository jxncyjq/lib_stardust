...
func Compress(data []byte, config *CompressionConfig) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.WithMessageF(err, "failed to create a compression writer: {{err}}", 100)
	}
	...
	if _, err = writer.Write(data); err != nil {
		return nil, errors.WithMessageF(err, "failed to compress input data: err: {{err}}", 100)
	}
...
func Decompress(data []byte) ([]byte, bool, error) {
	...
	if err != nil {
		return nil, false, errors.WithMessageF(err, "failed to create a compression reader: {{err}}", 100)
	}
	if reader == nil {
