...
func GetSubjKeyID(privateKey crypto.Signer) ([]byte, error) {
	if privateKey == nil {
		return nil, fmt.Errorf("passed-in private key is nil")
	}

	marshaledKey, err := x509.MarshalPKIXPublicKey(privateKey.Public())
	if err != nil {
		return nil, fmt.Errorf(fmt.Sprintf("error marshalling public key: %s", err))
	}
...
func ParsePKIJSON(input []byte) (*ParsedCertBundle, error) {
	result := &CertBundle{}
	err := json.DecodeJSON(input, &result)
	...
	var secret Secret
	err = json.DecodeJSON(input, &secret)

	if err == nil {
		return ParsePKIMap(secret.Data)
	}

	return nil, fmt.Errorf("unable to parse out of either secret data or a secret object")
}
...
func ParsePEMBundle(pemBundle string) (*ParsedCertBundle, error) {
	...
	for len(pemBytes) > 0 {
		...
		if pemBlock == nil {
			return nil, fmt.Errorf("no data found in PEM block")
		}

		if signer, err := x509.ParseECPrivateKey(pemBlock.Bytes); err == nil {
			if parsedBundle.PrivateKeyType != UnknownPrivateKey {
				return nil, fmt.Errorf("more than one private key given; provide only one private key in the bundle")
			}
			...
			return nil, fmt.Errorf("Encrypted private key given; provide only decrypted private key in the bundle")
		}
	}
	...
	if err := parsedBundle.Verify(); err != nil {
		return nil, fmt.Errorf(fmt.Sprintf("verification of parsed bundle failed: %s", err))
	}
...
func GeneratePrivateKey(keyType string, keyBits int, container ParsedPrivateKeyContainer) error {
	...
	case "rsa":
		...
		if err != nil {
			return fmt.Errorf(fmt.Sprintf("error generating RSA private key: %v", err))
		}
	...
	case "ec":
		...
		case 521:
			curve = elliptic.P521()
		default:
			return fmt.Errorf(fmt.Sprintf("unsupported bit length for EC key: %d", keyBits))
		}
		privateKey, err = ecdsa.GenerateKey(curve, rand.Reader)
		if err != nil {
			return fmt.Errorf(fmt.Sprintf("error generating EC private key: %v", err))
		}
		privateKeyBytes, err = x509.MarshalECPrivateKey(privateKey.(*ecdsa.PrivateKey))
		if err != nil {
			return fmt.Errorf(fmt.Sprintf("error marshalling EC private key: %v", err))
		}
	default:
		return fmt.Errorf(fmt.Sprintf("unknown key type: %s", keyType))
	}
...
func GenerateSerialNumber() (*big.Int, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(fmt.Sprintf("error generating serial number: %v", err))
	}
	return serial, nil
}
...
func ParsePublicKeyPEM(data []byte) (interface{}, error) {
	...
	return nil, errors.New("data does not contain any valid public keys", 0)
}
...
func CreateCertificate(data *CreationBundle) (*ParsedCertBundle, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error getting subject key ID: %s", err)
	}
	...
	if err := HandleOtherSANs(certTemplate, data.Params.OtherSANs); err != nil {
		return nil, errors.WithMessageF(err, "error marshaling other SANs: {{err}}", 0, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("unable to create certificate: %s", err)
	}

	result.CertificateBytes = certBytes
	result.Certificate, err = x509.ParseCertificate(certBytes)
	if err != nil {
		return nil, fmt.Errorf("unable to parse created certificate: %s", err)
	}
...
func CreateCSR(data *CreationBundle, addBasicConstraints bool) (*ParsedCSRBundle, error) {
	...
	if addBasicConstraints {
		...
		if err != nil {
			return nil, errors.WithMessageF(err, "error marshaling basic constraints: {{err}}", 0, err)
		}
		ext := pkix.Extension{
	...
	if err != nil {
		return nil, fmt.Errorf("unable to parse created certificate: %v", err)
	}
...
func SignCertificate(data *CreationBundle) (*ParsedCertBundle, error) {
	switch {
	case data == nil:
		return nil, fmt.Errorf("nil data bundle given to signCertificate")
	case data.Params == nil:
		return nil, fmt.Errorf("nil parameters given to signCertificate")
	case data.SigningBundle == nil:
		return nil, fmt.Errorf("nil signing bundle given to signCertificate")
	case data.CSR == nil:
		return nil, fmt.Errorf("nil csr given to signCertificate")
	}

	err := data.CSR.CheckSignature()
	if err != nil {
		return nil, fmt.Errorf("request signature invalid")
	}
	...
	if err := HandleOtherSANs(certTemplate, data.Params.OtherSANs); err != nil {
		return nil, fmt.Errorf("error marshaling other SANs: {{err}}", err)
	}
	...
	if data.Params.IsCA {
		...
		if data.SigningBundle.Certificate.MaxPathLen == 0 &&
			...
			return nil, fmt.Errorf("signing certificate has a max path length of zero, and cannot issue further CA certificates")
		}
...
func parseCertsPEM(pemCerts []byte) ([]*x509.Certificate, error) {
	...
	if !ok {
		return certs, errors.New("data does not contain any valid RSA or ECDSA certificates", 0)
	}
	return certs, nil
}
