// Package cert contains helper functions that are mostly used
...
func (c *CertBundle) ToParsedCertBundle() (*ParsedCertBundle, error) {
	...
	if len(c.PrivateKey) > 0 {
		...
		case PKCS8Block:
			...
			if err != nil {
				return nil, fmt.Errorf("Error getting key type from pkcs#8: %v", err)
			}
		...
		default:
			return nil, fmt.Errorf("Unsupported key block type: %s", pemBlock.Type)
		}
	...
	if len(c.Certificate) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("Error encountered parsing certificate bytes from raw bundle: %v", err)
		}
	}
	switch {
	case len(c.CAChain) > 0:
		for _, cert := range c.CAChain {
			...
			if err != nil {
				return nil, fmt.Errorf("Error encountered parsing certificate bytes from raw bundle via CA chain: %v", err)
			}
	...
	case len(c.IssuingCA) > 0:
		...
		if pemBlock == nil {
			return nil, fmt.Errorf("Error decoding ca certificate from cert bundle")
		}

		parsedCert, err := x509.ParseCertificate(pemBlock.Bytes)
		if err != nil {
			return nil, fmt.Errorf("Error encountered parsing certificate bytes from raw bundle via issuing CA: %v", err)
		}
...
func (p *ParsedCertBundle) getSigner() (crypto.Signer, error) {
	...
	if p.PrivateKeyBytes == nil || len(p.PrivateKeyBytes) == 0 {
		return nil, fmt.Errorf("Given parsed cert bundle does not have private key information")
	}
	...
	default:
		return nil, fmt.Errorf("Unable to determine type of private key; only RSA and EC are supported")
...
func getPKCS8Type(bs []byte) (PrivateKeyType, error) {
	...
	if err != nil {
		return UnknownPrivateKey, fmt.Errorf("Failed to parse pkcs#8 key: %v", err)
	}
	...
	default:
		return UnknownPrivateKey, fmt.Errorf("Found unknown private key type in pkcs#8 wrapping")
	}
}
...
func (c *CSRBundle) ToParsedCSRBundle() (*ParsedCSRBundle, error) {
	...
	if len(c.CSR) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("Error encountered parsing certificate bytes from raw bundle via CSR: %v", err)
		}
	}
